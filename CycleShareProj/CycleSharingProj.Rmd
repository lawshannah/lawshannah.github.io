...
```{r, eval = TRUE, label = "dayOfWeekSeasons", fig.align='center'}
start_date_ym %>%
  group_by(wd, Season) %>%
  summarize(N = n())%>%
...
```{r, echo = FALSE, eval=TRUE, label = "timeOfDaySeasons", fig.cap="Number of Trips by Time of Day"}
...
start_date_ym %>%
  ...
  theme_bw() +
  guides(fill = FALSE) +
  geom_vline(aes(xintercept = quantile(tripduration, prob = 0.75)), linetype = "dashed", data = start_date_ym)
```
